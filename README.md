...
| 删除 | Key 范围校验后的单条/批量删除；Paimon 墓碑隐藏数据并同步清理 MinIO 对象 |
...
访问 `http://localhost:8081`，服务会自动进入登录页；使用 `.env` 中的管理账号登录，创建绑定业务域和部门的
...
3. 自动创建资产描述符、文本切片/概要、图片特征、音频特征、传输审计和文件删除墓碑六张 Paimon 表；
...
通用全局索引同时要求关闭 deletion vectors，因此内容表采用追加模式。删除操作向
`multimodal_file_deletion` 写入墓碑记录，并从清单、详情、全文检索和向量检索中过滤对应
`file_id`；MinIO 原文件与缩略图同步清理。需要物理回收 Paimon 历史内容行时，应在维护窗口按
分区执行离线重写，避免破坏在线索引约束。
